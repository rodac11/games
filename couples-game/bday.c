...
const char *NAMELIST_PROGRAMS[] = {"MUSIC", "CALENDAR","ALARM"};
...
void p1_level0(){
	printf("\nRodrigo: Eh...another day...\n");
	dsleep(1);
	printf("\nHmm...coding...something...?");
	dsleep(3);
	...
	while(1){
		...
		printf("You have your NOTEBOOK and your LAPTOP. PANTERA is on your BED."
...
void notebook(){
  ...
  if(p.inventory[2].used==false){
	...
	case '1':
		printf("\nHere's your notes:\n");
		dsleep(1);
		printf("\n\t* I finished programming that CALENDAR app for Ira!"
			"\n\t I do hope she uses it soon...");
		dsleep(2);
		printf("\n\t* Man, I had a really cool idea, but I forgot all about it!"
			"\n\t Maybe Ira will remember?");
		dsleep(3);
		int tmp = -1;

		//ehh this will be hanging in there
		for(int i = 0; i < SPECIAL_SIZE; i++) {
			if(p.special_inventory[i].used == true){
				tmp = 0;
			}
		}
		printf("\nThat's all the notes we have!");
		break;
	case '2':
		printf("\nYou take out your trusty Dixon Ticonderoga pencil...");
		promptstring(&s);
		printf("\nWritten down!");
		break;
  ...
  void pet_cat(){
	  head();
	  if(p.inventory[2].used == false){
		  printf("\nYou pet Pantera's head. He purrs.");
		  ellipsis();
		  printf("\nMeow!");
		  ellipsis();
		  printf("\nOh! \nThe cat moved from where he was on the bed.");
		  dsleep(1);
		  printf("\nPANTERA was lying on your NOTEBOOK!");

		  p.inventory[2].used = false;
		  dsleep(3);
	  }
	  else{
		  printf("\nYou pet Pantera's head.");
		  ellipsis();
		  printf("\nHe's asleep, but he's still purring!");
		  dsleep(3);
	  }
...
void r_laptop(){
	...
	for(int i = 0; i < MISC_NAMELIST_SIZE; i++){
		if(strcmp(s, NAMELIST_PROGRAMS[i]) == 0){
			tmp = i;
		}
	}
	...
	switch(tmp){
	case 0:
		printf("\nYou coded a nice MUSIC application!");
		ellipsis();
		printf("\n8-bit music is coming out of your speakers!");
		ellipsis();
		printf("\nMEOW!");
		dsleep(1);
		printf("\nLooks like Pantera's hungry. I better go feed him!");
		complete_level();
		dsleep(3);
	}
}
